    ...
    <Layout>
      ...
      <section className="relative h-[75vh] flex items-end overflow-hidden">
        ...
        <div className="relative z-20 section-padding pb-16 lg:pb-24 w-full max-w-7xl mx-auto">
          <ScrollReveal>
            <Link href="/destinations" className="text-gold/60 hover:text-gold text-[10px] font-bold inline-flex items-center gap-3 mb-10 transition-colors uppercase tracking-[0.4em]">
              <ArrowLeft className="h-4 w-4" /> Back to Explorations
            </Link>
            <div className="flex items-center gap-4 mb-6">
              <div className="h-px w-12 bg-gold" />
              <span className="text-gold text-[10px] font-black uppercase tracking-[0.5em] italic">Private Dispatch</span>
            </div>
            <h1 className="font-heading text-6xl lg:text-9xl font-bold text-cream mb-8 uppercase tracking-tighter leading-[0.85]">{destination.name}</h1>
            <div className="flex flex-wrap gap-8 text-cream/90 font-body text-xs font-bold uppercase tracking-[0.3em]">
          ...
          <div className="lg:col-span-8 space-y-32">
            ...
            <ScrollReveal>
               <div className="flex items-center gap-4 mb-10">
                  ...
                  <h2 className="font-heading text-xs uppercase tracking-[0.4em] font-bold text-neutral-500">The Essence</h2>
               </div>
               <h3 className="font-heading text-5xl font-bold mb-12 uppercase tracking-tight text-white">Overview</h3>
               <div className="prose prose-invert prose-gold max-w-none">
                 <p className="font-body text-neutral-300 leading-relaxed text-2xl lowercase tracking-wider first-letter:uppercase first-letter:text-6xl first-letter:font-heading first-letter:text-gold first-letter:mr-4 first-letter:float-left">
            ...
            {itinerary.length > 0 && (
              <ScrollReveal>
                 <div className="flex items-center gap-4 mb-10">
                    ...
                    <h2 className="font-heading text-xs uppercase tracking-[0.4em] font-bold text-neutral-500">The Sequence</h2>
                 </div>
                 <h3 className="font-heading text-5xl font-bold mb-16 uppercase tracking-tight text-white">Itinerary</h3>
                 <div className="space-y-0">
                   {itinerary.map((item: any, idx: number) => (
                     <div key={idx} className="flex gap-12 border-l border-gold/10 pl-16 pb-20 last:pb-0 relative group">
                       <div className="absolute -left-[18px] top-0 w-9 h-9 bg-neutral-900 border border-gold/40 text-gold rounded-none flex items-center justify-center font-bold text-xs ring-8 ring-background transition-all group-hover:bg-gold group-hover:text-charcoal">
                         {idx + 1}
                       </div>
                       <div className="space-y-6">
                          <h4 className="font-heading text-3xl font-bold uppercase tracking-tight text-cream/90 group-hover:text-gold transition-colors">{item.day || `Day ${idx + 1}`}: {item.title}</h4>
                          <p className="font-body text-neutral-400 leading-relaxed text-lg lowercase tracking-wide italic">
            ...
            <ScrollReveal>
               <div className="flex items-center gap-4 mb-10">
                  ...
                  <h2 className="font-heading text-xs uppercase tracking-[0.4em] font-bold text-neutral-500">The Coordinates</h2>
               </div>
               <h3 className="font-heading text-5xl font-bold mb-12 uppercase tracking-tight text-white">Map Overview</h3>
               <div className="relative aspect-video bg-neutral-900 border border-gold/10 overflow-hidden flex items-center justify-center group">
                  ...
                  <div className="relative z-10 text-center space-y-4">
                     ...
                     <p className="text-gold text-[10px] uppercase font-bold tracking-[0.5em]">{destination.name} Coordinates</p>
                     <p className="text-neutral-500 font-mono text-xs">Lat: 1.2921° S, Long: 36.8219° E</p>
                  </div>
               </div>
            </ScrollReveal>
            ...
            {accommodations.length > 0 && (
              <ScrollReveal>
                 <div className="flex items-center gap-4 mb-10">
                    ...
                    <h2 className="font-heading text-xs uppercase tracking-[0.4em] font-bold text-neutral-500">The Havens</h2>
                 </div>
                 <h3 className="font-heading text-5xl font-bold mb-16 uppercase tracking-tight text-white">Featured Stays</h3>
                 <div className="grid md:grid-cols-2 gap-12">
                   {accommodations.map((acc: any) => (
                     <div key={acc.id} className="group bg-neutral-900/40 border border-gold/5 p-1 relative hover:border-gold/20 transition-all duration-700 overflow-hidden">
                        ...
                        <div className="p-8">
                           <div className="flex justify-between items-center mb-4">
                             <h4 className="font-heading text-2xl font-bold uppercase tracking-tight text-cream">{acc.name}</h4>
                             <div className="flex items-center text-gold">
                                {Array.from({ length: 5 }).map((_, i) => <Star key={i} className={`h-3 w-3 ${i < (acc.rating || 5) ? 'fill-gold' : 'text-neutral-700'}`} />)}
                             </div>
                           </div>
                           <p className="text-[10px] text-neutral-500 uppercase font-bold tracking-[0.2em] mb-0 flex items-center gap-3">
                              <MapPin className="h-4 w-4 text-gold/40" /> {acc.location}
                           </p>
                        </div>
                     </div>
                   ))}
